...
def test_hybrid_channel_first_instantiation(qua_config):
    ...
    hybrid = HybridChannelFirst(
        id="test_c1", opx_output=LFFEMAnalogOutputPort("con1", 1, 1)
    )
...
def test_hybrid_qubit_first_instantiation(qua_config):
    ...
    hybrid = HybridQubitFirst(
        id="test_q1", opx_output=LFFEMAnalogOutputPort("con1", 1, 2)
    )
...
def test_hybrid_int_id_naming():
    ...
    h1 = HybridChannelFirst(id=1, opx_output=LFFEMAnalogOutputPort("con1", 1, 1))
    # Channel._default_label is "ch"
    assert h1.name == "ch1"

    # Qubit first: defaults to "q{id}"
    h2 = HybridQubitFirst(id=2, opx_output=LFFEMAnalogOutputPort("con1", 1, 2))
    # Qubit logic
    assert h2.name == "q2"
...
def test_hybrid_with_root_config():
    ...
    root = MyRoot(
        component=HybridChannelFirst(
            id=1, opx_output=LFFEMAnalogOutputPort("con1", 1, 1)
        )
    )
